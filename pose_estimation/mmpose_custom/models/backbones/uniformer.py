...
class SABlock_Windows(nn.Module):
    def __init__(self, dim, num_heads, window_size=14, mlp_ratio=4.,
                 qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
...
class UniFormer(nn.Module):
    ...
    def __init__(self, layers=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=80, embed_dim=[64, 128, 320, 512],
        ...
        self.blocks1 = nn.ModuleList([
            CBlock(
                dim=embed_dim[0], num_heads=num_heads[0],
        ...
        self.blocks2 = nn.ModuleList([
            CBlock(
                dim=embed_dim[1], num_heads=num_heads[1],
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate,
                drop_path=dpr[i+layers[0]], norm_layer=norm_layer)
        ...
        if self.windows:
            ...
            SABlock_Windows(
                dim=embed_dim[2], num_heads=num_heads[2], window_size=window_size,
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate,
                drop_path=dpr[i+layers[0]+layers[1]], norm_layer=norm_layer)
        ...
        self.blocks4 = nn.ModuleList([
            SABlock(
                ...
                mlp_ratio=mlp_ratio,qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate,
                drop_path=dpr[i+layers[0]+layers[1]+layers[2]], norm_layer=norm_layer)
    ...
    def init_weights(self, pretrained):
        if isinstance(pretrained, str):
            ...
            if _state_dict.get('absolute_pos_embed') is not None:
                absolute_pos_embed = _state_dict['absolute_pos_embed']
                N1, L, C1 = absolute_pos_embed.size()
                N2, C2, H, W = self.absolute_pos_embed.size()
                if N1 != N2 or C1 != C2 or L != H*W:
                    logger.warning("Error in loading absolute_pos_embed, pass")
                else:
                    _state_dict['absolute_pos_embed'] = absolute_pos_embed.view(N2, H, W, C2).permute(0, 3, 1, 2)
