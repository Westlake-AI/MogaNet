...
def get_num_layer_layer_wise(var_name, num_max_layer=12):
    ...
    if var_name in ("backbone.cls_token", "backbone.mask_token", "backbone.pos_embed"):
        return 0
    elif var_name.startswith("backbone.patch_embed"):  # e.g., 'backbone.patch_embed1.projection.weight'
        stage_id = int(var_name.split('.')[1].split('patch_embed')[-1])
        if stage_id == 1:
            layer_id = 0
    ...
    elif var_name.startswith("backbone.blocks"):  # e.g., 'backbone.blocks3.1.attn.proj_1.weight'
        stage_id = int(var_name.split('.')[1].split('blocks')[-1])
...
class LearningRateDecayOptimizerConstructorMogaNet(DefaultOptimizerConstructor):
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        for name, param in module.named_parameters():
            ...
            if len(param.squeeze().shape) == 1 or name.endswith(".bias") or name in ('pos_embed', 'cls_token'):
                group_name = "no_decay"
                this_weight_decay = 0.
            else:
                group_name = "decay"
